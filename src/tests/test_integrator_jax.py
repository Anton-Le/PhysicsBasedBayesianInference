...
def harmonic_test(stepSize, numParticles, method):
    ...
    if method == 'Leapfrog':
        sol_q_p = Leapfrog(stepSize, finalTime, harmonicGradient)
    elif method == 'Stormer-Verlet':
        sol_q_p = StormerVerlet(stepSize, finalTime, harmonicGradient)
    else:
    ...
    print(f'{ensemble1.q=}')
    print(f'{ensemble1.p=}')
    print(f'{ensemble1.mass=}')
    q = jnp.array(ensemble1.q, dtype='float32')
    p = jnp.array(ensemble1.p, dtype='float32')
    mass = jnp.array(ensemble1.mass, dtype='float32')
    q_num, p_num = sol_q_p.integrate(q, p, mass)
...
def plotError():
    ...
    numParticles = 2 # clearest with one particle
    numDimensions = 1
