...
class Leapfrog(Integrator):
    def integrate(self, q, p, mass):
        ...
        body_func = lambda i, val: _leapfrogBodyFunc(
            i, val, self.stepSize, self.gradient, mass
        )
...
class StormerVerlet(Integrator):
    def integrate(self, q, p, mass):
        ...
        q = (
            q
            + v * self.stepSize
            - 0.5 * self.stepSize ** 2 * self.gradient(q) / mass
        )

        initial_val = (q, qPast)

        body_func = lambda i, val: _stormerVerletBodyFunc(
            i, val, self.stepSize, self.gradient, mass
        )
