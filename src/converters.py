...
class Converter:
    def __init__(self, model, modelArguments: tuple, modelData: dict):
        ...
        for paramName in trace.keys():
            ...
            if (paramProperties["type"] == "sample") and (
                paramProperties["is_observed"] is False
            ):
                self.parametersAndShapes[paramName] = paramProperties[
                    "value"
                ].size
    ...
    def toDict(self, parameterVector: np.array):
        ...
        assert (
            self.vectorSize == parameterVector.size
        ), "Vector size incorrect for the number of parameters"
