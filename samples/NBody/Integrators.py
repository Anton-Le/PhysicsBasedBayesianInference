...
######################################################################
def IntegrateHI(r_arg, v_arg, M_arg, G=1, tmax=100, dt=0.1, MaxRef=1, flag=False):
	r = np.copy(r_arg)
	v = np.copy(v_arg)
	M = np.copy(M_arg)
	Ntot = int( tmax/dt )
...
######################################################################
def IntegrateHIA(r_arg, v_arg, M_arg, G=1, tmax=100, dt=0.1, MaxRef=1, flag=False):
	r = np.copy(r_arg)
	v = np.copy(v_arg)
	M = np.copy(M_arg)
	dt0 = dt
...
#	print "Npart=", N
	...
	for i in range(NSteps):
		t[i+1]=t[i]+dt
		sol = fsolve(lambda p: y[i,:]+dt *NBodyODE( (y[i,:]+p)/2.0 ,M,G,N)-p, y[i+1,:])
		y[i+1,:]=sol
	if flag:
		return (t,y)
	else:
